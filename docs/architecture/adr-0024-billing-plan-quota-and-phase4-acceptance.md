...
`phase4:accept` gerçek PostgreSQL + pgvector ve pinli Codex `0.144.2` ile PDF
upload/index, workspace-local MCP retrieval, citation, mobil approval, session resume,
terminal usage/cost ve plan/budget görünümünü tek cleanup-safe senaryoda doğrular.
Delete/reindex, iki cihaz CAS, failed/interrupted accounting, hard quota ve
cross-tenant görünmezlik aynı gate içindedir. Billing ve Web Push credential yoksa
gerçek-provider opt-in smoke `not-run` raporlanır; emulator sonucu production kanıtı
sayılmaz.
