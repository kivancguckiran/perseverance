...
- `/healthz` yalnız process liveness döndürür. `/readyz` database, artifact storage, workspace, auth ve app-server bağımlılıklarını typed `ready|failed` check'lerle açıklar; değer, credential veya path dönmez.
