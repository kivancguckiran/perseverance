// Static self-hosted release invariants over real distribution files.
...
import {
  SELF_HOSTED_RELEASE_PINNED_IMAGE_PATTERN,
  SELF_HOSTED_RELEASE_REQUIRED_FILES,
  SELF_HOSTED_RELEASE_REQUIRED_SERVICES,
  checkComposeFile,
  checkImagesEnv,
  checkShellScript,
...
describe('self-hosted release dağıtım dosyaları', () => {
  it('gerekli tüm dosyalar mevcut', () => {
    const missing = SELF_HOSTED_RELEASE_REQUIRED_FILES.filter(
  ...
  it('yalnız workspace-agent nested sandbox için seccomp filtresini gevşetir', () => {
    ...
    const workspaceAgent = services.find(
      (service) => service.name === 'workspace-agent',
    )
    expect(workspaceAgent?.body).toContain('cap_drop:')
    expect(workspaceAgent?.body).toContain('- ALL')
    expect(workspaceAgent?.body).toContain('no-new-privileges:true')
    expect(workspaceAgent?.body).toContain('seccomp=unconfined')
    for (const service of services.filter(
      (service) => service.name !== 'workspace-agent',
    ))
      expect(service.body).not.toContain('seccomp=unconfined')
  })
  ...
  it('upgrade bind mount yollarını yeni release checkoutuna taşır', () => {
    const upgrade = extractShellFunction(
      read('infra/self-hosted/self-hosted.sh'),
      'cmd_upgrade',
    )
    ...
    expect(upgrade.indexOf('SELF_HOSTED_MIGRATIONS_DIR')).toBeLessThan(
      upgrade.indexOf('compose run --rm migrate'),
    )
  })
  ...
  it('product.Dockerfile digest-pinli base, non-root ve placeholder origin kullanır', () => {
    ...
    expect(dockerfile).toContain(
      'mkdir -p /codex-home/runtime /workspace /scoped-workspace',
    )
    expect(dockerfile).toContain(
  ...
  it('the self-hosted guide documents one-command installation and recovery limits', () => {
    const install = read('infra/self-hosted/README.md')
    expect(install).toContain('self-hosted.sh install')
    expect(install).toContain('backup')
    expect(install).toContain('recovery')
  })
})

describe('self-hosted-release-lib fonksiyonları', () => {
  ...
  it('pin deseni yalnız digest içeren referansları kabul eder', () => {
    expect(
      SELF_HOSTED_RELEASE_PINNED_IMAGE_PATTERN.test(
        `caddy:2.10.2@sha256:${'a'.repeat(64)}`,
      ),
    ).toBe(true)
    expect(SELF_HOSTED_RELEASE_PINNED_IMAGE_PATTERN.test('caddy:2.10.2')).toBe(
      false,
    )
    expect(SELF_HOSTED_RELEASE_PINNED_IMAGE_PATTERN.test('caddy:latest')).toBe(
      false,
    )
  })
  ...
  it('statik tarama: dağıtım kaynakları plaintext credential içermez', () => {
    const sources = SELF_HOSTED_RELEASE_REQUIRED_FILES.map((path) => ({
      name: path,
      content: read(path),
    }))
    expect(scanForCredentials(sources, selfHostedStaticScanPolicy)).toEqual([])
  })
  ...
  it('summarizeGates not-run sonucu asla başarıya terfi ettirmez', () => {
    const summary = summarizeGates([
      { gate: 'self-hosted release:test', accepted: true, status: 'passed' },
      {
        gate: 'self-hosted release:install-smoke',
        accepted: false,
        status: 'not-run',
      },
    ])
