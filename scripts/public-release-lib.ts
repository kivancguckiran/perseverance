...
// Public-release library. Deterministic helpers do not depend on wall-clock time.
...
// infra/release/public-gitleaks.toml için yeterlidir ve resmî gitleaks ile aynı
// dosyayı paylaşır.
...
export const parseGitleaksToml = (text: string): SecretPolicy => {
  ...
  for (const entry of policy.allowlists)
    if (!entry.description.trim())
      throw new Error(
        'public-release secret policy: her allowlist kaydı gerekçe (description) içermelidir',
      )
  return policy
}

// Offline koşularda gitleaks default kural setinin yüksek sinyalli yaklaşık
// karşılığı. Bu set preflight'ın her ortamda bağımsız çalışmasını sağlar.
...
export const SECRET_PATH_PROBES = [
  '.env',
  '.env.local',
  '.public-release/evidence/report.json',
  '.self-hosted-release/evidence/report.json',
  '.self-hosted-release/home/secrets/backup-key',
...
export const collectDependencyInventory = (
  ...
  // platform-specific: os/cpu kısıtlı platform varyantları ve yalnız bu
    ...
    if (verdict === 'unknown') {
      ...
      if (policy.unknownRequiresReview !== false && !reviewed)
        violations.push({
          ...
          license: component.license,
          reason:
            'bilinmeyen lisans; infra/release/license-policy.v1.json reviewedUnknown kaydı ve gerekçe gerekir',
        })
    }
  }
  return {
...
export const buildDependencySbom = (
  ...
  metadata: {
    ...
    properties: [
      { name: 'perseverance:generator', value: 'public-release-lib' },
...
export const REQUIRED_HYGIENE_FILES: { path: string; mustContain?: string }[] =
  [
    ...
    { path: 'third_party/openai-codex/LICENSE', mustContain: 'Apache License' },
    { path: 'third_party/openai-codex/NOTICE.md', mustContain: 'OpenAI Codex' },
    ...
    { path: 'SUPPORT.md' },
    { path: 'ARCHITECTURE.md', mustContain: 'Security model' },
  ]
...
export const checkHygieneFiles = (root: string): string[] => {
  ...
  for (const requirement of REQUIRED_HYGIENE_FILES) {
    ...
    if (!existsSync(absolute) || !statSync(absolute).isFile()) {
      problems.push(`${requirement.path}: missing`)
      continue
    }
    const text = readFileSync(absolute, 'utf8')
    if (!text.trim()) {
      problems.push(`${requirement.path}: empty`)
      continue
    }
    if (requirement.mustContain && !text.includes(requirement.mustContain))
      problems.push(
        `${requirement.path}: expected content missing (${requirement.mustContain})`,
      )
  }
  return problems
}
