...
describe('remote deploy idle gate', () => {
  const submitter = read('scripts/deploy-self-hosted-remote.sh')
  const worker = read('scripts/run-pending-self-hosted-deploy.sh')

  it('keeps both shell entrypoints syntactically valid', () => {
    for (const path of [
      'scripts/deploy-self-hosted-remote.sh',
  ...
  it('submits atomic pending state and detaches the remote worker', () => {
  ...
  it('contains no plaintext deployment credential', () => {
    expect(
      scanForCredentials(
        [
          { name: 'deploy-self-hosted-remote.sh', content: submitter },
