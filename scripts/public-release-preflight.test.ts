...
const policy = parseGitleaksToml(
  readFileSync(join(root, 'infra/release/public-gitleaks.toml'), 'utf8'),
)
...
const fakeMarker = ['PERSISTENT_PROVIDER_CREDENTIAL_', 'abcdefghijklmnop'].join(
  '',
)

describe('public-release secret policy', () => {
  it('default kuralları genişletir ve gerekçeli allowlist taşır', () => {
  ...
  it('gerçek görünümlü secret sınıflarını yakalar', () => {
    ...
    expect(ruleIds).toContain('provider-credential-marker')
  })
  ...
  it('test fixture yollarını ve sentinel değerleri allowlist ile eler', () => {
    ...
    expect(
      scanContent(
        'scripts/realtime-boundary.test.ts',
  ...
  it("working tree ve tam history taraması seed edilmiş secret'ı bulur", () => {
    const work = mkdtempSync(join(tmpdir(), 'public-release-scan-'))
    try {
      const git = (...args: string[]) =>
        execFileSync('git', args, { cwd: work })
      git('init', '--quiet')
      git('config', 'user.email', 'public-release@test.local')
      git('config', 'user.name', 'public-release')
...
describe('public-release gitignore secret kapsamı', () => {
  it('lokal secret ve artifact dizinleri gitignore kapsamındadır', () => {
    expect(checkGitignoreCoverage(root)).toEqual([])
  }, 15_000)
})

describe('public-release lisans gate ve SBOM', () => {
  ...
  it('commit edilmiş SBOM ve lisans raporu günceldir (drift yok)', () => {
    ...
    expect(
      readFileSync(join(root, 'infra/release/sbom.cdx.json'), 'utf8'),
    ).toEqual(sbom)
    expect(
      readFileSync(
        join(root, 'infra/release/dependency-license-report.json'),
        'utf8',
      ),
    ).toEqual(report)
  })
...
describe('public-release hijyen dosyaları', () => {
  it('temiz checkout install adımını non-interactive çalıştırır', () => {
    const preflight = readFileSync(
      join(root, 'scripts/public-release-preflight.ts'),
      'utf8',
    )
  ...
  it('eksik dosyayı raporlar', () => {
    const work = mkdtempSync(join(tmpdir(), 'public-release-hygiene-'))
    try {
